...
CLASS zcl_llm_tests_openrouter IMPLEMENTATION.
  ...
  METHOD simple_call.
    " Setup: this maps to openrouter google/gemini-flash-1.5

    DATA(client) = zcl_llm_factory=>get_client( 'gemini-flash-1.5' ).
    DATA(request) = client->new_request( ).

    request-metadata-caller = 'zcl_llm_test_openrouter'.
  ...
  METHOD multi_call.
    " Setup: this maps to openrouter google/gemini-flash-1.5 and anthropic/claude-3.5-haiku
    DATA(client) = zcl_llm_factory=>get_client( 'gemini-flash-1.5' ).
    DATA(request) = client->new_request( ).
    ...
    DATA(haiku_clnt) = zcl_llm_factory=>get_client( 'claude-3.5-haiku' ).
