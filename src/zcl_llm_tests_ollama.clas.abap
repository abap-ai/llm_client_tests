...
CLASS zcl_llm_tests_ollama IMPLEMENTATION.
  ...
  METHOD simple_call.
    " Setup: this maps to ollama llama3.2:3b-instruct-q8_0

    DATA(client) = zcl_llm_factory=>get_client( 'llama3.2' ).
    DATA(request) = client->new_request( ).

    request-metadata-caller = 'zcl_llm_test_ollama'.
  ...
  METHOD so_simple.
    ...
    DATA: BEGIN OF dog,
          ...
          END OF dog.
    DATA: dogs         LIKE STANDARD TABLE OF dog WITH EMPTY KEY,
    ...
    request-structured_output->set_schema( data = dogs description = descriptions ).
  ...
  METHOD multi_call.
    " Setup: this maps to ollama llama3.2:3b-instruct-q8_0 and deepseek-coder-v2
    DATA(client) = zcl_llm_factory=>get_client( 'llama3.2' ).
    DATA(request) = client->new_request( ).
    ...
    DATA(qwen_clnt) = zcl_llm_factory=>get_client( 'qwen2.5-coder' ).
    DATA(qwen_request) = qwen_clnt->new_request( ).
    APPEND LINES OF request-messages TO qwen_request-messages.
    APPEND VALUE #( role = response-choices[ 1 ]-message-role content = response-choices[ 1 ]-message-content ) TO qwen_request-messages.
